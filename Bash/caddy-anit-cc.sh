...
#单位时间[1分钟]内相同ip访问次数超过 60 次自动加入到 Cloudflare 防火墙.

ip=`cat $logfile/log_ip_top | awk '{if($1>60) print $2}'
...
curl -s -X POST "https://api.cloudflare.com/client/v4/zones/$ZoneID/firewall/access_rules/rules" \
  ...
  --data '{"mode":"block","configuration":{"target":"ip","value":"'$IPAddr'"},"notes":"恶意攻击"}'
done
